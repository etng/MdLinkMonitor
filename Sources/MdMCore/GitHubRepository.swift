...
public enum GitRepositoryParser {
    public static func parse(from rawURL: String, allowedDomains: Set<String>) -> GitRepository? {
        let normalizedURL = URLNormalizer.normalizedURLForDedup(rawURL)
...
public enum MarkdownTaskLineBuilder {
    public static func makeLine(label: String, repositoryURL: String, annotation: String? = nil) -> String {
        let base = "* [ ] [\(label)](\(repositoryURL))"
        guard
            let annotation,
        ...
        let normalizedAnnotation = annotation
            ...
            .filter { !$0.isEmpty }
            .joined(separator: " ")
        guard !normalizedAnnotation.isEmpty else {
            return base
        }

        return "\(base) - \(normalizedAnnotation)"
    }
}
