...
final class MenuBarViewModel: ObservableObject {
    ...
    @Published private(set) var recentCaptureFeedbackKind: CaptureFeedbackKind = .none
    ...
    private lazy var autoMergeWindowsManager: AutoMergeWindowsManager = {
        AutoMergeWindowsManager { [weak self] message in
    ...
    private var captureFeedbackHideWorkItem: DispatchWorkItem?
    private var autoUpdateWorkItem: DispatchWorkItem?
    private var isSyncingDailyReferenceLinks = false
    ...
    private enum SyncStateKeys {
        static let lastDailyReferenceSyncThroughYMD = "mdmonitor.dailyReferenceLinks.lastThroughYMD"
    }
    ...
    private struct DailyReferenceSyncRun: Sendable {
        let throughDate: Date
        let throughYMD: String
        let result: DailyReferenceLinkSyncResult
    }
    ...
    init(
        ...
        let fileLogger = DailyFileLogger(
            baseDirectoryPath: loaded.outputDirectoryPath,
            retentionDays: loaded.logRetentionDays
        )
        self.memoryLogger = memoryLogger
        self.fileLogger = fileLogger
        self.logger = CompositeLogger(loggers: [memoryLogger, fileLogger])
        fileLogger.pruneOldLogsIfNeeded(force: true)
        ...
        configureAutoMergeWindows()
        triggerDailyReferenceSyncIfNeeded(reason: "startup")
    }
    ...
    var commandLineToolName: String {
        CommandLineToolInstaller.executableName
    }
    ...
    func updateLaunchAtLogin(_ enabled: Bool) {
        ...
        if enabled, let failureReason = SystemIntegrationGuard.launchAtLoginFailureReason() {
            let message = launchAtLoginUnavailableMessage(for: failureReason)
            setStatus(message)
    ...
    func installCommandLineTool() {
        ...
        switch CommandLineToolInstaller.resolveBundledExecutablePath() {
        case .success(let path):
            executablePath = path
        case .failure(let failureReason):
            let message = commandLineToolUnavailableMessage(for: failureReason)
            setStatus(message)
            logger.log(.error, message)
            showToast(message)
            return
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                ...
                let commandName = self.commandLineToolName

                switch result {
                case .installed(let path, let requiredAdmin):
                    let message = requiredAdmin
                        ? self.local(
                            "\(commandName) 已安装（已完成管理员授权）",
                            "\(commandName) installed (administrator authorization completed)"
                        )
                        : self.local("\(commandName) 已安装", "\(commandName) installed")
                ...
                case .cancelled:
                    let message = self.local(
                        "安装 \(commandName) 已取消",
                        "\(commandName) installation canceled"
                    )
                    self.setStatus(message)
                ...
                case .failed(let reason):
                    let message = self.local(
                        "安装 \(commandName) 失败，请检查权限或路径",
                        "Failed to install \(commandName). Check permission and path."
                    )
                    self.setStatus(message)
    ...
    func preferredObsidianVault() -> ObsidianVaultLocation? {
    ...
    func saveSettings(_ nextDraft: AppSettings) {
        ...
        if previous.launchAtLogin != next.launchAtLogin {
            if next.launchAtLogin,
               let failureReason = SystemIntegrationGuard.launchAtLoginFailureReason() {
                next.launchAtLogin = previous.launchAtLogin
                let warn = launchAtLoginUnavailableMessage(for: failureReason)
                setStatus(warn)
                logger.log(.warning, warn)
                showToast(warn)
            } else if !launchAtLoginManager.setEnabled(next.launchAtLogin) {
                next.launchAtLogin = previous.launchAtLogin
                let warn = local(
                    "开机启动设置失败，已保留原设置",
        ...
        if previous.outputDirectoryPath != next.outputDirectoryPath ||
            previous.logRetentionDays != next.logRetentionDays {
            fileLogger.baseDirectoryPath = next.outputDirectoryPath
            fileLogger.retentionDays = next.logRetentionDays
            fileLogger.pruneOldLogsIfNeeded(force: true)
            if previous.outputDirectoryPath != next.outputDirectoryPath {
                reloadRecentFiles()
            }
        ...
        if previous.autoMergeMacWindowsEnabled != next.autoMergeMacWindowsEnabled ||
            previous.autoMergeMacWindowsBundleIDs != next.autoMergeMacWindowsBundleIDs {
            configureAutoMergeWindows()
        }

        triggerDailyReferenceSyncIfNeeded(reason: "settingsSaved")
    ...
    func openTodayMainWindow() {
        triggerDailyReferenceSyncIfNeeded(reason: "openTodayMainWindow")
    ...
    func openMainWindowCalendar() {
        triggerDailyReferenceSyncIfNeeded(reason: "openMainWindowCalendar")
        let path = openTodayFilePath()
    ...
    func openAbout() {
        triggerDailyReferenceSyncIfNeeded(reason: "openAbout")
    ...
    func openSettings() {
        triggerDailyReferenceSyncIfNeeded(reason: "openSettings")
    ...
    func openUpdatesPanel() {
        triggerDailyReferenceSyncIfNeeded(reason: "openUpdatesPanel")
    ...
    func checkForUpdates(userInitiated: Bool = true) -> AppUpdateCheckResult {
        triggerDailyReferenceSyncIfNeeded(reason: userInitiated ? "manualCheckForUpdates" : "automaticCheckForUpdates")
        if userInitiated {
    ...
    private func handleClipboardText(_ text: String) {
        triggerDailyReferenceSyncIfNeeded(reason: "clipboard")
        ...
        let metadataDomains = Set(settingsSnapshot.metadataFetchAllowedDomains)
        let repositoryDomains = Set(settingsSnapshot.repositoryDomains)
        let absoluteClipboardURL = AbsoluteWebURL.normalize(text)
        let attachmentURLCandidate = absoluteClipboardURL.flatMap { ClipboardContentProcessor.isAttachmentURL($0) ? $0 : nil }
        let plainURLCandidate = ClipboardContentProcessor.extractWhitelistedPlainURL(
            from: text,
            allowedDomains: metadataDomains
        )
        let specialRepositoryCandidate = ClipboardContentProcessor.extractSpecialPlainRepositoryCapture(
            from: text,
            repositoryDomains: repositoryDomains
        )

        if extractedLinks.isEmpty && plainURLCandidate == nil && attachmentURLCandidate == nil && specialRepositoryCandidate == nil {
            let hint = local("未识别到可处理的 Markdown 链接、附件链接或白名单 URL", "No processable markdown link, attachment URL, or allowlisted URL detected")
            setStatus(hint)
            logger.log(.info, hint)
            return
        }

        if let attachmentURLCandidate {
            logger.log(.info, "Attachment URL detected host=\(URLComponents(string: attachmentURLCandidate)?.host ?? "")")
        }

        if let plainURLCandidate {
            logger.log(.info, "Allowlisted plain URL detected host=\(plainURLCandidate.host ?? "")")
        }

        if case let .link(link) = specialRepositoryCandidate {
            logger.log(.info, "Special plain repository URL detected: \(link.markdownURL)")
        }
        ...
        captureProcessingQueue.async { [weak self] in
            ...
            let result = orchestrator.process(
                ...
                repositoryDomains: repositoryDomains,
                metadataAllowedDomains: metadataDomains,
            ...
            DispatchQueue.main.async {
                ...
                guard result.totalCandidates > 0 else {
                    let hint = self.local(
                        "已识别内容，但没有可写入的链接或附件",
                        "Detected content but nothing was written or saved"
                    )
                    self.setStatus(hint)
                ...
                let summary = self.captureSummary(for: result)

                self.setStatus(summary)
                self.logger.log(.info, summary)
                if result.feedbackKind != .none {
                    self.triggerCaptureFeedback(kind: result.feedbackKind)
                }
    ...
    private func configureRestAPIServer() {
        ...
        restAPIServer.apply(
            ...
            dailyReferenceSyncHandler: { [weak self] date, completion in
                Task { @MainActor [weak self] in
                    guard let self else {
                        completion(
                            Self.makeRESTResponse(
                    ...
                    self.processRESTDailyReferenceSync(dateString: date, completion: completion)
                }
            },
    ...
    private func configureAutoMergeWindows() {
        autoMergeWindowsManager.apply(
            enabled: settings.autoMergeMacWindowsEnabled,
            targetBundleIDs: settings.autoMergeMacWindowsBundleIDs
        )
    }
        ...
        captureProcessingQueue.async { [weak self] in
            ...
            if result.totalCandidates == 0 {
                response = Self.makeRESTResponse(
                    ...
                    jsonObject: [
                        "ok": false,
                        "message": "No processable links or attachments detected under current settings",
                    ]
                )
            } else {
                response = Self.makeRESTResponse(
            ...
            DispatchQueue.main.async {
                ...
                if result.totalCandidates == 0 {
                    let hint = self.local(
                        "REST 请求未识别到可处理的链接或附件",
                        "REST request did not contain processable links or attachments"
                    )
                    self.setStatus(hint)
                    self.logger.log(.info, hint)
                } else {
                    let summary = self.captureSummary(
                        for: result,
                        prefix: self.local("REST 处理", "REST processed")
                    )
                    self.setStatus(summary)
                    self.logger.log(.info, summary)
                    if result.feedbackKind != .none {
    ...
    private func processRESTDailyReferenceSync(
        dateString: String?,
        completion: @escaping @Sendable (LocalRESTAPIServerResponse) -> Void
    ) {
        guard !isSyncingDailyReferenceLinks else {
            completion(
                Self.makeRESTResponse(
                    statusCode: 503,
                    jsonObject: [
                        "ok": false,
                        "error": "daily reference sync already in progress",
                    ]
                )
            )
            return
        ...
        let throughResolution: (date: Date, clamped: Bool)
        do {
            throughResolution = try resolveDailyReferenceSyncThroughDate(from: dateString)
        } catch {
            completion(
                Self.makeRESTResponse(
                    statusCode: 400,
        ...
        _ = runDailyReferenceSync(
            throughDate: throughResolution.date,
            reason: "rest",
            ...
            case .success(let run):
                completion(
                    Self.makeRESTResponse(
                        ...
                        jsonObject: [
                            "ok": true,
                            "through": Self.restAPIDateFormatter.string(from: run.throughDate),
                            "clamped": throughResolution.clamped,
                            "processedFiles": run.result.processedFiles,
                            "updatedFiles": run.result.updatedFiles,
                            "skippedFiles": run.result.skippedFiles,
                            "keptLinks": run.result.keptLinks,
                            "appendedLinks": run.result.appendedLinks,
                        ]
                    )
                )
            case .failure(let error):
                self.logger.log(.error, "REST daily reference sync failed: \(error.localizedDescription)")
                completion(
                    Self.makeRESTResponse(
                        statusCode: 500,
                        jsonObject: [
    ...
    private func updateSettings(_ mutation: (inout AppSettings) -> Void) {
        ...
        settings = next
        settingsStore.save(next)
        triggerDailyReferenceSyncIfNeeded(reason: "settingsChanged")
    }
    ...
    private func captureSummary(for result: CaptureProcessResult, prefix: String? = nil) -> String {
        let otherSkippedCount = max(result.skippedCount - result.duplicateCount - result.blockedCount, 0)

        let zhParts = [
            "识别 \(result.totalCandidates) 项",
            "写入 \(result.appendedCount)",
            "附件 \(result.attachmentCount)",
            "克隆 \(result.clonedCount)",
            result.duplicateCount > 0 ? "重复 \(result.duplicateCount)" : nil,
            result.blockedCount > 0 ? "已阻止 \(result.blockedCount)" : nil,
            otherSkippedCount > 0 ? "跳过 \(otherSkippedCount)" : nil
        ].compactMap { $0 }

        let enParts = [
            "detected \(result.totalCandidates)",
            "appended \(result.appendedCount)",
            "attachments \(result.attachmentCount)",
            "cloned \(result.clonedCount)",
            result.duplicateCount > 0 ? "duplicates \(result.duplicateCount)" : nil,
            result.blockedCount > 0 ? "blocked \(result.blockedCount)" : nil,
            otherSkippedCount > 0 ? "skipped \(otherSkippedCount)" : nil
        ].compactMap { $0 }

        let body = local(zhParts.joined(separator: "，"), enParts.joined(separator: ", "))
        guard let prefix, !prefix.isEmpty else {
            return body
        }
        return prefix + local("：", ": ") + body
    }
    ...
    private func triggerCaptureFeedback(kind: CaptureFeedbackKind, duration: TimeInterval = 1.6) {
        guard kind != .none else {
            return
        }

        captureFeedbackHideWorkItem?.cancel()
        recentCaptureFeedbackKind = kind

        let workItem = DispatchWorkItem { [weak self] in
            guard let self else { return }
            if self.recentCaptureFeedbackKind == kind {
                self.recentCaptureFeedbackKind = .none
            }
        }
        captureFeedbackHideWorkItem = workItem
        DispatchQueue.main.asyncAfter(deadline: .now() + duration, execute: workItem)
    }
    ...
    private func launchAtLoginUnavailableMessage(for reason: SystemIntegrationGuard.FailureReason) -> String {
        switch reason {
        case .developmentBuild:
            return local(
                "当前是开发构建，不能设置开机启动。请从 /Applications 或 ~/Applications 中已安装的 MdMonitor.app 操作。",
                "Launch at login is unavailable in development builds. Open the installed MdMonitor.app from /Applications or ~/Applications."
            )
        case .appNotInstalled:
            return local(
                "当前应用不在 /Applications 或 ~/Applications，不能设置开机启动。请先安装正式 app。",
                "Launch at login requires MdMonitor.app to be installed in /Applications or ~/Applications."
            )
        case .standaloneExecutable:
            return local(
                "当前不是从 app 包启动，不能设置开机启动。请先安装正式 app。",
                "Launch at login is unavailable for standalone executables. Install and run the packaged app first."
            )
        case .missingBundledTool:
            return local(
                "当前应用安装不完整，无法设置开机启动。",
                "The current app bundle is incomplete, so launch at login cannot be configured."
            )
        }
    }

    private func commandLineToolUnavailableMessage(for reason: SystemIntegrationGuard.FailureReason) -> String {
        let commandName = commandLineToolName
        switch reason {
        case .developmentBuild:
            return local(
                "当前是开发构建，不能安装 \(commandName) 命令。请从 /Applications 或 ~/Applications 中已安装的 app 操作。",
                "Installing \(commandName) is unavailable in development builds. Open the installed app from /Applications or ~/Applications."
            )
        case .appNotInstalled:
            return local(
                "当前应用不在 /Applications 或 ~/Applications，不能安装 \(commandName) 命令。请先安装 app。",
                "Installing \(commandName) requires the app to be installed in /Applications or ~/Applications."
            )
        case .standaloneExecutable:
            return local(
                "当前不是从 app 包启动，不能安装 \(commandName) 命令。请先安装 app。",
                "Installing \(commandName) is unavailable for standalone executables. Install and run the packaged app first."
            )
        case .missingBundledTool:
            return local(
                "当前 app 包中缺少 \(commandName) 可执行文件，请重新安装当前版本。",
                "The current app bundle does not include the \(commandName) executable. Reinstall the packaged app."
            )
        }
    }
    ...
    func syncDailyReferenceLinksNow() {
        guard let throughDate = dailyReferenceSyncThroughDate() else {
            return
        }
        _ = runDailyReferenceSync(
    ...
    private func triggerDailyReferenceSyncIfNeeded(reason: String) {
        guard let throughDate = dailyReferenceSyncThroughDate() else {
            return
        }
        let throughYMD = DailyMarkdownStore.ymdString(from: throughDate)
        let lastYMD = defaults.string(forKey: SyncStateKeys.lastDailyReferenceSyncThroughYMD) ?? ""
        guard lastYMD < throughYMD else {
            return
        }
        triggerDailyReferenceSync(force: false, reason: reason)
    }

    private func triggerDailyReferenceSync(force: Bool, reason: String) {
        guard let throughDate = dailyReferenceSyncThroughDate() else {
            return
        }
        ...
        let throughYMD = DailyMarkdownStore.ymdString(from: throughDate)
        let lastYMD = defaults.string(forKey: SyncStateKeys.lastDailyReferenceSyncThroughYMD) ?? ""
        if !force, lastYMD >= throughYMD {
            return
        }
        _ = runDailyReferenceSync(
            throughDate: throughDate,
            reason: reason,
            forceStatusUpdate: force,
            completion: nil
        )
    }

    private func dailyReferenceSyncThroughDate(now: Date = Date()) -> Date? {
        let startOfToday = Calendar(identifier: .gregorian).startOfDay(for: now)
        return Calendar(identifier: .gregorian).date(byAdding: .day, value: -1, to: startOfToday)
    }

    private func runDailyReferenceSync(
        throughDate: Date,
        reason: String,
        forceStatusUpdate: Bool,
        completion: (@MainActor @Sendable (Result<DailyReferenceSyncRun, Error>) -> Void)?
    ) -> Bool {
        guard !isSyncingDailyReferenceLinks else {
            return false
        }
        ...
        let throughYMD = DailyMarkdownStore.ymdString(from: throughDate)
        let settingsSnapshot = settings
        let cbmDirectoryPath = settingsSnapshot.outputDirectoryPath
        let dailyRootDirectoryPath = settingsSnapshot.dailyReferenceDirectoryPath
        let referenceSectionHeading = settingsSnapshot.dailyReferenceSectionHeading

        isSyncingDailyReferenceLinks = true
        logger.log(
            .info,
            "Start daily reference link sync reason=\(reason), through=\(throughYMD), cbm=\(cbmDirectoryPath), dailyRoot=\(dailyRootDirectoryPath), heading=\(referenceSectionHeading)"
        )

        captureProcessingQueue.async { [weak self] in
            let synchronizer = DailyReferenceLinkSynchronizer(
                cbmDirectoryPath: cbmDirectoryPath,
                dailyRootDirectoryPath: dailyRootDirectoryPath,
                referenceSectionHeading: referenceSectionHeading
            )

            do {
                let result = try synchronizer.sync(through: throughDate)
                let run = DailyReferenceSyncRun(throughDate: throughDate, throughYMD: throughYMD, result: result)
                DispatchQueue.main.async {
                    guard let self else { return }
                    self.isSyncingDailyReferenceLinks = false
                    self.persistDailyReferenceSyncThroughYMD(throughYMD)

                    let summary = self.local(
                        "Daily 链接同步完成：更新 \(result.updatedFiles) 天，追加 \(result.appendedLinks) 条",
                        "Daily link sync finished: updated \(result.updatedFiles) days, appended \(result.appendedLinks) links"
                    )
                    self.logger.log(
                        .info,
                        "Daily link sync finished reason=\(reason), through=\(throughYMD), processed=\(result.processedFiles), updated=\(result.updatedFiles), skipped=\(result.skippedFiles), kept=\(result.keptLinks), appended=\(result.appendedLinks)"
                    )
                    if result.updatedFiles > 0 || forceStatusUpdate {
                        self.setStatus(summary)
                    }
                ...
                DispatchQueue.main.async {
                    ...
                    let message = self.local(
                        "Daily 链接同步失败，请查看日志",
                        "Daily link sync failed, check logs"
                    )
                    self.setStatus(message)
                    self.logger.log(.error, "Daily link sync failed reason=\(reason): \(error.localizedDescription)")
                    self.notifyIfEnabled(message)
    ...
    private func persistDailyReferenceSyncThroughYMD(_ throughYMD: String) {
        let previous = defaults.string(forKey: SyncStateKeys.lastDailyReferenceSyncThroughYMD) ?? ""
        defaults.set(max(previous, throughYMD), forKey: SyncStateKeys.lastDailyReferenceSyncThroughYMD)
    }

    private func resolveDailyReferenceSyncThroughDate(from dateString: String?) throws -> (date: Date, clamped: Bool) {
        let trimmed = dateString?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !trimmed.isEmpty else {
            if let date = dailyReferenceSyncThroughDate() {
                return (date, false)
            }
            throw NSError(
                domain: "MdMonitor.DailyReferenceSync",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "unable to resolve yesterday"]
            )
        }

        guard let parsedDate = Self.restAPIDateFormatter.date(from: trimmed) else {
            throw NSError(
                domain: "MdMonitor.DailyReferenceSync",
                code: 2,
        ...
        guard let yesterday = dailyReferenceSyncThroughDate() else {
            throw NSError(
                domain: "MdMonitor.DailyReferenceSync",
                code: 3,
                userInfo: [NSLocalizedDescriptionKey: "unable to resolve yesterday"]
