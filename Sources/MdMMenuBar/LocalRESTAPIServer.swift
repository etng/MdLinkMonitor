...
final class LocalRESTAPIServer: @unchecked Sendable {
    ...
    typealias DailyReferenceSyncHandler = (_ date: String?, _ completion: @escaping @Sendable (LocalRESTAPIServerResponse) -> Void) -> Void
    ...
    private var dailyReferenceSyncHandler: DailyReferenceSyncHandler?
    ...
    func apply(
        ...
        dailyReferenceSyncHandler: DailyReferenceSyncHandler?,
        statusHandler: StatusHandler?
    ) {
        self.captureHandler = captureHandler
        self.dailyHandler = dailyHandler
        self.dailyReferenceSyncHandler = dailyReferenceSyncHandler
    ...
    private func handle(request: HTTPRequest, on connection: NWConnection) {
        ...
        if request.method == "POST", request.path == "/api/v1/sync-daily-md-links" {
            guard let dailyReferenceSyncHandler else {
                send(
                    jsonResponse(statusCode: 503, jsonObject: ["error": "daily reference sync handler unavailable"]),
                    on: connection
                )
                return
            }

            dailyReferenceSyncHandler(extractDate(from: request)) { [weak self] response in
                guard let self else {
                    connection.cancel()
                    return
                }
    ...
    private func extractDate(from request: HTTPRequest) -> String? {
        if let date = request.query["through"] ?? request.query["date"] {
        ...
        if contentType.contains("application/json"),
           let object = try? JSONSerialization.jsonObject(with: request.body) as? [String: Any] {
            if let date = (object["through"] as? String) ?? (object["date"] as? String) {
                let trimmed = date.trimmingCharacters(in: .whitespacesAndNewlines)
                if !trimmed.isEmpty {
                    return trimmed
                }
